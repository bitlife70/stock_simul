#!/usr/bin/env python3
"""
Simplified Integration Test Suite for Korean Stock Backtesting Platform
...
class SimpleIntegrationTest:
    """Simplified integration test for Korean Stock Backtesting Platform"""
    ...
    def test_component_architecture(self):
        ...
        for component in backend_components:
            ...
            if exists:
                print(f"✅ {component} - {size/1024:.1f}KB")
            else:
        ...
        for api_name, base_url in self.api_endpoints.items():
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ {api_name} - {response_time:.0f}ms")
                else:
                    health_results[api_name] = {
                        'status': 'unhealthy',
                ...
                print(f"❌ {api_name} - Server offline")
        ...
        for api_name, base_url in self.api_endpoints.items():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ Stock list: {len(stocks_data) if isinstance(stocks_data, list) else 0} stocks")
                else:
                    api_results['stock_list'] = {'status': 'failed', 'code': response.status_code}
                    print(f"❌ Stock list failed: HTTP {response.status_code}")
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ Stock data: {len(stock_data) if isinstance(stock_data, list) else 0} data points")
                else:
                    api_results['stock_data'] = {'status': 'failed', 'code': response.status_code}
                    print(f"❌ Stock data failed: HTTP {response.status_code}")
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ Strategy templates: {len(strategies) if isinstance(strategies, list) else 0} strategies")
                else:
                    api_results['strategies'] = {'status': 'failed', 'code': response.status_code}
                    print(f"❌ Strategy templates failed: HTTP {response.status_code}")
                
                data_results[api_name] = api_results
                
            except Exception as e:
                data_results[api_name] = {'error': str(e)}
                print(f"❌ Data integration failed for {api_name}: {e}")
    ...
    def test_backtesting_engine(self):
        ...
        try:
            ...
            print(f"✅ Standalone backtest: {result.get('total_return', 0):.2%} return")
        ...
        for api_name, base_url in self.api_endpoints.items():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ API backtest: {result.get('total_return', 0):.2%} return")
                else:
                    backtest_results[api_name] = {'status': 'failed', 'code': response.status_code}
                    print(f"❌ API backtest failed: HTTP {response.status_code}")
    ...
    def test_korean_features(self):
        ...
        for symbol in test_symbols:
            ...
            print(f"{'✅' if is_valid else '❌'} {symbol} - {symbol_tests[symbol]['market']}")
        ...
        print("✅ Korean market hours configured")
    ...
    def test_frontend_components(self):
        ...
        for component_name, file_path in frontend_files.items():
            ...
            if exists:
                ...
                try:
                    ...
                    print(f"✅ {component_name} - {size/1024:.1f}KB")
                    ...
                    print(f"⚠️ {component_name} - {size/1024:.1f}KB (read error)")
            else:
                frontend_results[component_name] = {
                    'exists': False,
                    'status': 'missing'
        ...
        if package_json_path.exists():
            try:
                ...
                print(f"✅ Dependencies: {len(key_dependencies) - len(missing_deps)}/{len(key_dependencies)} key deps")
                
            except Exception as e:
                frontend_results['dependencies'] = {'error': str(e)}
                print(f"❌ Package.json read error: {e}")
        else:
